...
-   Los modelos lineales y lineales generalizados asumen independencia entre observaciones. Ignorar la correlación dentro de los clústeres puede llevar a subestimar los errores estándar y, en consecuencia, a realizar inferencias incorrectas.
...
-   De este modo, el modelo permite capturar la dependencia entre observaciones mediante la inclusión explícita de componentes aleatorios que representan la variabilidad entre clústeres.
...
-   Los modelos mixtos se componen de dos partes:
    ...
    -   **Efectos aleatorios:** variabilidad adicional atribuible a los clústers. Modelan la correlación intra-clúster y representan la variabilidad no explicada entre unidades del mismo grupo.

        ::: {.callout-warning appearance="simple"}
        Los efectos fijos describen la tendencia promedio en la población, mientras que los efectos aleatorios capturan la variabilidad entre los distintos clústeres.
        :::
...
-   Las **diferencias entre clústeres** reflejan distintos promedios de la respuesta y permiten modelar la heterogeneidad entre grupos sin estimar un parámetro específico para cada uno.
...
-   El dataset `sleepstudy`, incluido en `lme4`, contiene información sobre el tiempo de reacción diario en 18 personas con privación de sueño.
...
-   En el ejemplo de [regresión lineal múltiple](https://datos-ine.github.io/curso_avanzada_2025/unidad_3/07_reg_lineal_multiple.html), la tasa de mortalidad tenía asociaciones significativas con `mediana_edad`, `mediana_ingresos`, `pct_desempleo` y `estado`.
...
##### LMM con `estado` como intercepto aleatorio
...
-   Además, permite considerar los estados como una muestra aleatoria de una población mayor de posibles unidades geográficas.
...
En el ejemplo anterior, podríamos considerar que `condado` y `estado` son efectos aleatorios anidados, entonces, podemos plantear el siguiente modelo para aquellos condados con más de 2 observaciones:

```{r}
#| id: RLM multinivel
# Filtrar condados con más de 2 observaciones
cancer2 <- cancer |>
group_by(condado) |> 
filter(n() > 2) 

# Modelo mixto anidado
mod_multi <- glmmTMB(
  tasa_mortalidad ~ mediana_ingresos + mediana_edad +  pct_desempleo +
    (1 | estado / condado),  data = cancer2
)
```
...
```{r}
#| id: Salida modelo anidado
parameters(mod_multi, include_info = TRUE)
```

-   Podemos observar que el componente de varianza se divide en tres partes para reflejar la estructura anidada de los datos:
    -   Varianza entre estados
    -   Varianza entre condados del mismo estado
    -   Varianza residual
...
-   Permiten modelar la **dependencia intra-clúster** sin violar el supuesto de independencia de los residuos al introducir correlación estructurada mediante efectos aleatorios.

------------------------------------------------------------------------

#### Ejemplo: Incidencia de dengue en la Costa Atlántica de PBA

-   En la [Unidad 5](https://datos-ine.github.io/curso_avanzada_2025/unidad_5/03_dispersion.html), usamos el dataset `dengue_costa.txt` para ejemplificar datos de conteo con sobredispersión.
...
-   Ahora, usaremos estos datos para ajustar un GLMM que controle las medidas repetidas por departamento
...
-   Al incorporar un intercepto aleatorio por departamento, el modelo reconoce que la incidencia de dengue puede diferir sistemáticamente entre unidades geográficas, más allá de las covariables incluidas.
