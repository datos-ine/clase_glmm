...
Supongamos que los pacientes del ejemplo anterior se distribuyen en tres hospitales:

```{r}
#| id: Crear variable para hospital
sleepstudy <- sleepstudy |>
mutate(hospital = case_when(
    between(subject, "308", "332") ~ "Hosp1",
    between(subject, "333", "350") ~ "Hosp2",
    .default = "Hosp3"
))

# Tabla frecuencias
tabyl(sleepstudy, hospital) |>
adorn_pct_formatting()
```

------------------------------------------------------------------------

Ajustamos el modelo multinivel:

```{r}
#| id: LMM multinivel
mod5 <- glmmTMB(reaction ~ days + (1 | hospital/subject), data = sleepstudy)

parameters(mod5, include_info = TRUE)
```

-   El término `(1|hospital/subject)` indica que los pacientes están anidados en hospitales.
